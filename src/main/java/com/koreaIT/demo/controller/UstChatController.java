...
public class UstChatController {
	...
	/*
	  ...
	  결론적으로, /topic/messages는 메시지를 브로드캐스트하기 위한 '가상의' 채널이며, 이 채널은 스프링 애플리케이션의 메모리 내부에서 관리됩니다. 
	  클라이언트는 이 채널을 구독함으로써 서버로부터 실시간으로 메시지를 받을 수 있게 됩니다.	
	 */
    ...
    public ChatMessage sendMessage(@Payload ChatMessage chatMessage) {
		 ...
		 // 여기서 메시지를 처리하고, 결과를 '/topic/messages'로 브로드캐스트합니다.
